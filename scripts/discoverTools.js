...
function validateSchema(schemaPath) {
    try {
        ...
        if (!valid) {
            const errors = toolSchemaValidator.errors?.map(err => `${err.instancePath} ${err.message}`) || ['Unknown validation error'];
...
function discoverTools(srcDir, schemasDir) {
...
function generateToolManifest(tools, outputPath) {
    const manifest = {
...
//# sourceMappingURL=discoverTools.js.map