...
export class S3Explorer {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, bucket:string, key?:string) {
        ui.logToOutput('S3Explorer.constructor Started');

        this.SetS3ExplorerItem(bucket, key);
    ...
    public SetS3ExplorerItem(bucket:string, key?:string){
        this.S3ExplorerItem = new S3ExplorerItem(bucket, key || "");
    }
    ...
    public static Render(extensionUri: vscode.Uri, bucket:string, key?:string, changeKey:string|undefined=undefined) {
        ui.logToOutput('S3Explorer.Render Started');
        if (S3Explorer.Current) {
            S3Explorer.Current.ResetCurrentState();
            S3Explorer.Current.SetS3ExplorerItem(bucket, key);
        ...
        else 
        {
            ...
            S3Explorer.Current = new S3Explorer(panel, extensionUri, bucket, key);
        }
        if(changeKey)
        {
